// src/injected/page.ts
var MOBILE_VIEWPORT = { width: 414, height: 896 };
var originalUserAgent = navigator.userAgent;
var originalPlatform = navigator.platform;
var ORIGINAL_VIEWPORT = document.querySelector("meta[name=viewport]")?.getAttribute("content") || "";
function overrideUserAgent(enabled) {
  if (!("__defineGetter__" in Navigator.prototype)) {
    return;
  }
  if (enabled) {
    Navigator.prototype.__defineGetter__(
      "userAgent",
...
function applyViewport(enabled) {
...
async function initSession() {
  ...
  const appId = shared?.config?.viewer?.app_id || getMetaContent("instagram:app_id") || "936619743392459";
  const rolloutHash = shared?.rollout_hash || initialData?.data?.rollout_hash || "";
  const userId = shared?.config?.viewer?.id || null;
  const claim = getCookie("ig_www_claim") || initialData?.data?.ig_www_claim || "0";
...
async function getVideoMetadata(file) {
  return new Promise((resolve, reject) => {
    ...
    video.onloadedmetadata = () => {
      const durationMs = Math.round(video.duration * 1e3);
...
function updateClaimFromHeaders(session, headers) {
  if (!headers) {
    return;
  }
...
async function uploadViaPage(file, requestId) {
  ...
  const videoMeta = file.type.startsWith("video/") ? await getVideoMetadata(file) : null;
  ...
  await new Promise((resolve, reject) => {
    ...
    xhr.onload = () => {
      const claimHeader = xhr.getResponseHeader("x-ig-set-www-claim") || xhr.getResponseHeader("x-ig-www-claim");
      if (claimHeader) {
...
window.addEventListener("message", (event) => {
  ...
  if (event.data?.type === "ISU_UPLOAD") {
    ...
    uploadViaPage(file, requestId).then(() => {
...
async function configureStory(session, uploadId, requestId, videoMeta) {
  const maxAttempts = 6;
  for (let attempt = 0; attempt < maxAttempts; attempt += 1) {
    const configureResponse = await fetch(
      "https://www.instagram.com/api/v1/media/configure_to_story/",
      {
        ...
        body: new URLSearchParams({
          ...
          client_shared_at: `${Math.floor(Date.now() / 1e3)}`,
          timezone_offset: `${-(/* @__PURE__ */ new Date()).getTimezoneOffset() * 60}`,
          camera_position: "back",
          caption: "",
          poster_frame_index: "0",
          length: videoMeta ? `${Math.round(videoMeta.durationMs / 1e3)}` : "0",
    ...
    if (configureData.status === "ok" && message !== "media_needs_reupload") {
      const mediaId = configureData.media?.id || configureData.media?.pk || configureData.media_id || null;
      if (mediaId) {
        window.postMessage(
          {
            type: "ISU_STATUS",
...
async function uploadCoverPhoto(session, uploadId, coverBlob, meta) {
